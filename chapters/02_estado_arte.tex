...
La automatización permite garantizar consistencia y reducir errores humanos, especialmente en entornos dinámicos. Según Rahman y Williams (2019) \cite{rahman2019devsecops}, la integración temprana de la seguridad mejora significativamente la resiliencia del software.
...
Estas prácticas suponen un riesgo elevado, especialmente en entornos \textit{multi-tenant} —donde múltiples equipos, aplicaciones o clientes comparten un mismo clúster Kubernetes con aislamiento lógico mediante namespaces—, ya que un compromiso en una carga de trabajo puede propagarse al resto si no existen controles adecuados de red, ejecución y privilegios.
...
Los Admission Controllers son mecanismos del API Server que interceptan las solicitudes antes de que los recursos sean desplegados en el clúster \cite{kubernetes_docs}. Permiten aplicar validaciones o modificaciones sobre los objetos.
