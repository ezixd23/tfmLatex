...
La aplicación de demostración es una aplicación Python Flask mínima cuya función es validar el ciclo CI/CD completo, no representar una carga de trabajo de producción real. Expone dos endpoints en el puerto~8080:
...
\begin{enumerate}
  ...
  \item \textbf{Generate SBOM with Syft}: generación del Software Bill of Materials en formato syft-json (el formato nativo de Syft \cite{syft_docs}), produciendo el fichero \texttt{sbom.syft.json}.
  ...
  \item \textbf{Vulnerability scan with Grype}: análisis del SBOM con Grype \cite{grype_docs}, configurado para fallar ante vulnerabilidades de severidad \texttt{HIGH} o superior con corrección disponible (\texttt{--only-fixed}).
...
El pipeline emite el SBOM en formato \textbf{syft-json}, el formato nativo de Syft, que Grype consume directamente para el análisis de vulnerabilidades. El artefacto generado (\texttt{sbom.syft.json}) se sube al workflow bajo el nombre \texttt{sbom-\$\{SHORT\_SHA\}}, asociándolo al run concreto y a la imagen correspondiente. A diferencia de SPDX o CycloneDX, syft-json no es un estándar de interoperabilidad reconocido, sino el formato interno de la herramienta; en un entorno con requisitos de compliance formales, el pipeline podría emitir adicionalmente el SBOM en CycloneDX mediante \texttt{-o cyclonedx-json}. La Figura~\ref{fig:ci-sbom-upload} muestra la ejecución exitosa del paso \textit{Upload SBOM artifact}: el fichero \texttt{sbom-defe4ef.zip} fue subido con un tamaño final de 490.279~bytes (ID de artefacto \texttt{7196257107}), accesible en la interfaz de artefactos del workflow \cite{github_actions_docs}.
...
\subsection{Incidencia 2: reproducibilidad frente a actualidad de la base de datos de Grype}
...
Este comportamiento expone un \textit{trade-off} fundamental en el diseño de pipelines de seguridad: la \textbf{reproducibilidad} (versión fijada de la imagen, comportamiento determinístico del pipeline) entra en conflicto con la \textbf{actualidad} de la base de datos (cobertura de las vulnerabilidades más recientes). Priorizar la reproducibilidad implica que el pipeline usará siempre la misma versión de Grype con la misma base de datos, pero esa base de datos se vuelve obsoleta con el tiempo. Priorizar la actualización de la base de datos implica que la imagen de Grype se actualiza automáticamente, pero introduce variabilidad en el comportamiento del pipeline (una ejecución puede fallar no porque la imagen tenga nuevas vulnerabilidades, sino porque la base de datos actualizada incorpora nuevas entradas).
...
Una vez detectado este nuevo commit por ArgoCD mediante su mecanismo de \textit{polling} del repositorio (y su \textit{watch} continuo sobre los recursos del clúster descrito en la Sección~7.5.1), el controlador de reconciliación inicia la sincronización del \texttt{Deployment} con la nueva especificación de imagen. La Figura~\ref{fig:ci-quay-tags} muestra el histórico y estado del repositorio de imágenes en Quay.io, donde ahora se observan las cuatro etiquetas publicadas por las distintas ejecuciones del pipeline: destacando \texttt{d8ae0bd} como la imagen más reciente (46.9~MiB), junto con las versiones anteriores \texttt{036fe39}, \texttt{defe4ef} (imagen con \texttt{python:3.13-slim}, 46.9~MiB) y \texttt{4888886} (imagen inicial con \texttt{python:3.12-slim}, 47.2~MiB).

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{ci_cd/bloque4_nuevoTAG.png}
\caption{Repositorio \texttt{zelkarrabi/tfm-demo-app} en Quay.io mostrando el histórico de las cuatro etiquetas publicadas por el pipeline, con \texttt{d8ae0bd} como la imagen más reciente y segura.}
